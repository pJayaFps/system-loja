...
import { getBrandPromos } from '../utils/api';
...
function PromoShowcase() {
  ...
  useEffect(() => {
    getBrandPromos().then((data) => setBrandCards(Array.isArray(data) ? data : []));
  }, []);
    ...
    <section className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2">
        {heroCards.map((card) => (
          <article key={card.titulo} className="rounded-2xl bg-black p-6 text-white shadow-soft">
            ...
            <button className="mt-4 rounded-full bg-white px-4 py-2 text-sm font-semibold text-black" onClick={() => goToFilter(card.categoria, card.subcategoria)}>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {brandCards.map((brand) => (
          <article key={brand.id} className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-soft">
            <img src={brand.imagem} alt={brand.nome} className="h-28 w-full object-cover" />
            <div className="p-4">
              <h4 className="font-bold">{brand.nome}</h4>
              <p className="mt-1 text-sm text-zinc-600">{brand.descricao || brand.desconto}</p>
              <button className="mt-3 rounded-full border border-black px-3 py-1 text-xs font-semibold" onClick={() => goToFilter(brand.categoria, brand.subcategoria)}>
                Comprar agora
              </button>
            </div>
          </article>
