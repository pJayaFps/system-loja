  ...
  return (
    <header className="fixed inset-x-0 top-0 z-[120] border-b border-zinc-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4 md:gap-4 md:px-6">
        ...
        <Link to="/" className="text-lg font-extrabold tracking-tight text-black md:text-xl">
          SportVault
        </Link>
        ...
        <div className="flex items-center gap-2 text-zinc-700 md:gap-3">
          <button className="hidden rounded-full border border-zinc-300 p-2 hover:bg-zinc-100 md:inline-flex" aria-label="Login">
            <User className="size-4" />
          </button>
          <button className="hidden rounded-full border border-zinc-300 p-2 hover:bg-zinc-100 md:inline-flex" aria-label="Favoritos">
              ...
              {categories.map((cat) => (
                <li key={cat.nome} className="rounded-xl border border-zinc-200 p-2">
                  <button
                    ...
                    onClick={() => setOpenCategory((prev) => (prev === cat.nome ? null : cat.nome))}
                  >
                    {cat.nome}
                    ...
                    <div className="mt-2 space-y-1 pl-2">
                      ...
                      {cat.subcategorias.map((sub) => (
                        <button
                          ...
                          onClick={() => filterBySubcategory(cat.nome, sub)}
                        >
                          {sub}
                        </button>
                      ))}
